...
select year(a.orderdate) as `Year`, month(a.orderdate) as `Month` , a.shipmethodid, b.`Name`, 
count(distinct a.salesorderid) as Order_count,
Avg(count(distinct a.salesorderid)) over (partition by year(a.orderdate), a.shipmethodid, b.`Name`) as Avg_Per_Month,
avg(count(distinct a.salesorderid)) over (Partition by a.shipmethodid, b.`Name`) as Avg_Per_Year 
from salesorderheader as a 
join shipmethod as b        on      a.shipmethodid = b.shipmethodid
group by `year`, `Month`, a.shipmethodid, b.`Name`  
order by `year`, `Month`, a.shipmethodid, b.`Name` ;
...
select year(a.orderdate) as `Year`, month(a.orderdate) as `Month`, b.cardtype as Card_Type,
 count(salesorderid) as Orders_Count,
 min(count(distinct salesorderid)) over (partition by Year(a.orderdate),b.cardtype) as Min_Order_Per_Month, 
 min(count(distinct salesorderid)) over (partition by b.cardtype) as Min_Order_Per_Year,
 max(count(distinct salesorderid)) over (partition by Year(a.orderdate),b.cardtype) as Max_Order_Per_Month, 
 max(count(distinct salesorderid)) over (partition by b.cardtype) as Max_Order_Per_Year
from salesorderheader as a
join creditcard as b         on a.creditcardid = b.creditcardid   
group by `Year`, `Month`,  b.cardtype                                               
order by `Year`, `Month`,  b.cardtype ;   
